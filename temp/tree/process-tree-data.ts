...
export function childNodeIsExpanded(node: TreeNode, expandKeys: string[]): boolean {
...
export function flatTree(treeData: TreeNode[], expandKeys?: string[]): TreeNode[] {
  let result: TreeNode[] = [];
  ...
  for (const node of treeData) {
    ...
    newNode.expanded = Array.isArray(expandKeys) && expandKeys.includes(newNode.key);
    result.push(newNode);
    if (node.childNodes && node.childNodes.length && childNodeIsExpanded(node, expandKeys || [])) {
...
export function getParentNodes(treeData: TreeNodeProp[], key: string): TreeNodeProp[] {
