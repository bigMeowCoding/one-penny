    ...
    <Fragment>
      ...
      <div className={addPrefixForClassName('')}>
        ...
        {props.footer && props.footer.length ? (
          <footer className={addPrefixForClassName('footer')}>{props.footer}</footer>
        ) : null}
      </div>
    </Fragment>
  );
...
export const confirm = (message: string, onOk?: () => void, onCancel?: () => void) => {
  ...
  const { close } = modal(
    message,
    [<button onClick={confirmHandle}>确认</button>, <button onClick={cancelHandle}>关闭</button>],
    onCancel,
  );
};
...
export const modal = (
  ...
  function close() {
    ReactDOM.render(React.cloneElement(dialogComponent, { visible: false }), div);
