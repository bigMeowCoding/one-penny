...
const Wave: FC<Props> = function ({ children, isInline, waveColor }) {
  ...
  return (
    <div
      ref={ref}
      className={
        isInline ? classNames(addClassByPrefix('inline-box')) : classNames(addClassByPrefix('box'))
      }
      onClick={() => {
        if (!ref.current) {
          return;
        }
        clickWaveTimerId = waveClickHandle(ref.current, styleRef.current, waveColor);
      }}
    >
      {children}
    </div>
