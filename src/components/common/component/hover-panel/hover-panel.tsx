...
const PanelContent: FC<PanelContentProp> = ({ position, overlay, ...props }) => {
  if (!position) {
    return null;
  }
  const positionStyle = { left: position.left, top: position.top };
  const styleProp = props.style ? { ...props.style, ...positionStyle } : positionStyle;
...
const HoverPanel: FC<Props> = ({ children, overlay, onMouseOver, layVisible, ...props }) => {
  ...
  return (
    <>
      ...
      {layVisible ? (
        <PanelContent overlay={overlay} {...props} position={positionRef.current} />
      ) : null}
    </>
  );
};
